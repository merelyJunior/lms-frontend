        ...
        {steps.map((item) => (
          <React.Fragment key={item.id}>
            ...
            >
              <div
                className={`grid  aspect-square w-[34px] place-items-center rounded-full border-[1px] 
                    ...
                    : "border-gray  "}
                    ${step > item.id && "bg-yellow-50 text-black text-yellow-50"}} `}
              >
                {step > item.id ?
            ...
            {item.id !== steps.length && (
              <div
                className={`h-[calc(34px/2)] w-[33%] border-dashed border-b-2 ${step > item.id ? "border-yellow-50" : ""} `}
              >
              </div>
            )}
          </React.Fragment>
      ...
      <div className="relative mb-16 flex w-full select-none justify-between">
        {steps.map((item) => (
          <div className={`sm:min-w-[130px] flex flex-col items-center gap-y-2 ${editCourse && 'sm:min-w-[270px]'}`} key={item.id}>
            <p className={`text-sm ${step >= item.id ? "text-white" : "text-white00"}`}>
              {item.title}
            </p>
          </div>
        ))}
